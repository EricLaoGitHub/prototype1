...
void HVSlicingNode::printChildren()
{
  for (vector<SlicingNode*>::iterator it = _children.begin(); it != _children.end(); it++)
    {
      if (((*it)->getType() == DeviceNode)||((*it)->getType() == RoutingNode)){ (*it)->print(); }
      else
        {
          (*it)->print();
          (*it)->printChildren();
      ...
      if (this->getType() == Vertical)
        ...
        // childrenpair represents 1 state of the combinations
          ...
          for (vector<SlicingNode*>::const_iterator it = _children.begin(); it != _children.end(); it++)
            { 
              if ( (*it)->getType() != RoutingNode ) { childrenpair.push_back(pair<float,float>((*it)->getMapHW()->begin()->first,(*it)->getMapHW()->begin()->second)); }
              else 
            ...
            // Need to be reset at each loop
              ...
              for (vector<SlicingNode*>::const_iterator it = _children.begin(); it != _children.end(); it++)
                ...
                // Collect the pair<float,float> of the combination in order sort them. Calculate the width of the combination.
                  if ((*it)->getType() != RoutingNode ) { currentHs.push_back(pair<float,float>((*itpair).first,(*itpair).second)); }
                      ...
                      if ( (*it)->getMapHW()->upper_bound((*itpair).first) != (*it)->getMapHW()->end() )
                        {
                          childrenpair[index] = pair<float,float>(
                                                                  (*it)->getMapHW()->upper_bound((*itpair).first)->first,
                                                                  (*it)->getMapHW()->upper_bound((*itpair).first)->second
                                                                 );
                        }
                      else
                        {
                          childrenpair[index] = pair<float,float>(
                                                                  (*it)->getMapHW()->begin()->first,
                                                                  (*it)->getMapHW()->begin()->second
                                                                 );
              ...
              if ((hmax-hmin) <= _toleranceH)
                {
                  if ( _mapHW->find(hmax) != _mapHW->end() )
                    {
                      if ( (*_mapHW->find(hmax)).second < currentW ) {( *_mapHW->find(hmax)).second = currentW; }
                    }
                  else { _mapHW->insert(pair<float,float> ( hmax, currentW) ); }
                }
            }
        }
      else if (this->getType() == Horizontal)
              ...
              if ((wmax-wmin) <= _toleranceW)
                {
                  if ( _mapHW->find(currentH) != _mapHW->end() )
                    {
                      if ( (*_mapHW->find(currentH)).second < wmax ){ (*_mapHW->find(currentH)).second = wmax; }
                    }
                  else 
...
// Find the closest (not higher) possible height for every children. Doing so will provide the lowest width possible.
  if ((this->getType() == Vertical)&&(this->emptyChildrenMap() != true))
    ...
    // childrenpair represents 1 state of the combinations, make sure that we take into account if the HVSlicingNode is preset.
      for (vector<SlicingNode*>::const_iterator it = _children.begin(); it != _children.end(); it++)
        {   
          if ((*it)->getPreset() != true) { childrenpair.push_back(pair<float,float>((*it)->getMapHW()->begin()->first,(*it)->getMapHW()->begin()->second)); }
          else 
            { childrenpair.push_back(pair<float,float>((*it)->getHeight(),(*it)->getWidth())); }
          index++;
        }
...
{ cerr << " Error(void DSlicingNode::addNode(SlicingNode* node, int index)): Devices do not have child." << endl; }
...
RHVSlicingNode* RHVSlicingNode::create(
                                   float             x, 
                                   float             y, 
                                   float             w, 
                                   float             h
                                  ){ return new RHVSlicingNode(x,y,w,h); }
...
void RHVSlicingNode::addNode(SlicingNode* node, int index)
{ cerr << " Error(void RHVSlicingNode::addNode(SlicingNode* node, int index)): RoutingNodes do not have child." << endl; }
